...
sugar_img = cv2.imread('CaramelColourStages/Sugar.jpg')
...
# Converting the image to a grayscale so that less information needs to be provided for each pixel. I think it's already in greyscale but just to be sure

sugar_img_gray = cv2.cvtColor(sugar_img, cv2.COLOR_BGR2GRAY)

# Showing the grayscale image

# cv2.imshow("Gray image", sugar_img_gray)
#
# cv2.waitKey(0)
...
while (capture.isOpened()):
    ...
    sugar_vid_gray = cv2.cvtColor(sugar_vid, cv2.COLOR_BGR2GRAY)
